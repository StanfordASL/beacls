    ...
    if (argc > 1) {
        ...
        while (ss >> d) {
            if (i == 0) {    // match size of precomputed initial conditions
                gN[i] = sizeof(InitialCondition::YTh)/sizeof(InitialCondition::YTh[0]);
            } else if (i == 1) {
                gN[i] = sizeof(InitialCondition::YTh[0])/sizeof(InitialCondition::YTh[0][0]);
            } else {
                gN[i] = d;
            }
            i++;
...
//  ''|...'|  .||.  '|' .||. .||...|'     |'....|'  .||. ||......| .||.....|
    ...
    const beacls::FloatVec gMin = beacls::FloatVec{ (FLOAT_TYPE)(-3),
                                                    (FLOAT_TYPE)(-M_PI/2),
                                                    (FLOAT_TYPE)1,
                                                    (FLOAT_TYPE)(-2),
                                                    (FLOAT_TYPE)(-1) };
    ...
    std::cout << "  y_rel: [" << gMin[0] << ", " << gMax[0] << "] (" << gN[0] << ")" << std::endl;
    std::cout << "psi_rel: [" << gMin[1] << ", " << gMax[1] << "] (" << gN[1] << ")" << std::endl;
    std::cout << "     Ux: [" << gMin[2] << ", " << gMax[2] << "] (" << gN[2] << ")" << std::endl;
    std::cout << "     Uy: [" << gMin[3] << ", " << gMax[3] << "] (" << gN[3] << ")" << std::endl;
    std::cout << "      r: [" << gMin[4] << ", " << gMax[4] << "] (" << gN[4] << ")" << std::endl;
...
// .|.  .||.     |      ''|...|'  .||. .||...|'     |'....|'  .||.....|   .||.
    ...
    // START INITIALIZATION
        ...
        for (size_t i = 0; i<num_of_elements; ++i) {
            ...
            // }
            size_t y_ind  = i % 13;
            size_t th_ind  = (i / 13) % 9;
            data0[i] = InitialCondition::YTh[y_ind][th_ind];
        }
    // END INITIALIZATION
    ...
    sD->dynSys = new helperOC::BicycleWall(beacls::FloatVec{(FLOAT_TYPE)0.,
                                                              (FLOAT_TYPE)0.,
                                                              (FLOAT_TYPE)0.,
                                                              (FLOAT_TYPE)0.,
                                                              (FLOAT_TYPE)0.},
                                                              beacls::IntegerVec{ 0,1,2,3,4});
