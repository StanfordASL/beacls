...
namespace BicycleCAvoid_CUDA {
	bool optCtrl_execute_cuda(
		...
		FLOAT_TYPE* d_dev_ptr = (d_ptr);
		FLOAT_TYPE* Fx_dev_ptr = (Fx_ptr);
		Fx_uvec.set_cudaStream(d_uvec.get_cudaStream());
		const FLOAT_TYPE* Ux_dev_ptr = (Ux_ptr);
		const FLOAT_TYPE* Uy_dev_ptr = (Uy_ptr);
		const FLOAT_TYPE* r_dev_ptr = (r_ptr);
		const FLOAT_TYPE* lam_Ux_dev_ptr = (lam_Ux_ptr);
		const FLOAT_TYPE* lam_Uy_dev_ptr = (lam_Uy_ptr);
		const FLOAT_TYPE* lam_r_dev_ptr = (lam_r_ptr);
		if ((uMode == helperOC::DynSys_UMode_Max) || (uMode == helperOC::DynSys_UMode_Min)) {
			...
			if (beacls::is_cuda(lam_Ux_uvec) && beacls::is_cuda(lam_Uy_uvec) && beacls::is_cuda(lam_r_uvec)) {
				for (size_t index = 0; index < d_uvec.size(); ++index) {
					const FLOAT_TYPE Ux = Ux_dev_ptr[index];
					const FLOAT_TYPE Uy = Uy_dev_ptr[index];
					const FLOAT_TYPE r = r_dev_ptr[index];
					const FLOAT_TYPE lam_Ux = lam_Ux_dev_ptr[index];
					const FLOAT_TYPE lam_Uy = lam_Uy_dev_ptr[index];
					const FLOAT_TYPE lam_r = lam_r_dev_ptr[index];
					...
					d_dev_ptr[index] = dOpt;
					Fx_dev_ptr[index] = FxOpt;
				}
			}
	...
	public:
			...
			void operator()(Tuple v) const
		{
			...
			FLOAT_TYPE X1w_per_v_max_lowspeed = X1::w_per_v_max_lowspeed;    // not sure why I have to do this??
			if (lam_norm < 0.001) {    // fuk tuples, i.e., I ain't debranching this
				...
				FLOAT_TYPE maxAy = X1w_per_v_max_lowspeed * V * V;                        // also bounded by maxA
				if (desAx > maxAx) {
	...
	struct Get_optDstb_scalars {
	public:
		const FLOAT_TYPE sign;
		const FLOAT_TYPE lam_w;
		const FLOAT_TYPE lam_Ax;
		Get_optDstb_scalars(const FLOAT_TYPE sign,
			const FLOAT_TYPE lam_w,
			const FLOAT_TYPE lam_Ax) : sign(sign), lam_w(lam_w), lam_Ax(lam_Ax) {}
		template<typename Tuple>
		__host__ __device__
			void operator()(Tuple v) const
		{
	...
	bool optDstb_execute_cuda(
		...
		FLOAT_TYPE* w_dev_ptr = (w_ptr);
		FLOAT_TYPE* a_dev_ptr = (a_ptr);
		a_uvec.set_cudaStream(w_uvec.get_cudaStream());
		const FLOAT_TYPE* V_dev_ptr = (V_ptr);
		...
		if ((dMode == helperOC::DynSys_DMode_Max) || (dMode == helperOC::DynSys_DMode_Min)) {
			...
			if (beacls::is_cuda(lam_w_uvec) && beacls::is_cuda(lam_Ax_uvec)) {
				const FLOAT_TYPE* lam_w_dev_ptr = (lam_w_ptr);
				const FLOAT_TYPE* lam_Ax_dev_ptr = (lam_Ax_ptr);
				for (size_t index = 0; index < w_uvec.size(); ++index) {
					const FLOAT_TYPE V = V_dev_ptr[index];
					const FLOAT_TYPE lam_w = lam_w_dev_ptr[index];
					const FLOAT_TYPE lam_Ax = lam_Ax_dev_ptr[index];
					FLOAT_TYPE lam_Ay = lam_w / V;
					FLOAT_TYPE lam_norm = hypot_float_type<FLOAT_TYPE>(lam_Ax, lam_Ay);
					FLOAT_TYPE maxA = X1::maxA_approx;
					FLOAT_TYPE X1maxAx = X1::maxAx;    // not sure why I have to do this??
					...
					if (lam_norm < 0.001) {    // fuk tuples, i.e., I ain't debranching this
						...
						if (desAx > maxAx) {
							...
							w_dev_ptr[index] = copysign_float_type<FLOAT_TYPE>(maxAy, desAy) / V;
							a_dev_ptr[index] = maxAx;
						}
						else {
							if (abs_float_type<FLOAT_TYPE>(desAy) > maxAy) {
								if (desAx > 0) {
									...
									w_dev_ptr[index] = copysign_float_type<FLOAT_TYPE>(maxAy, desAy) / V;
									a_dev_ptr[index] = -sqrt_float_type<FLOAT_TYPE>(maxA * maxA - maxAy * maxAy);
								}
							}
							else {
								w_dev_ptr[index] = desAy / V;
								a_dev_ptr[index] = maxAx;
							}
						}
					}
	...
	bool dynamics_cell_helper_execute_cuda_dimAll(
		...
		FLOAT_TYPE* dx_rel_dev_ptr = (dx_rel_ptr);
		FLOAT_TYPE* dy_rel_dev_ptr = (dy_rel_ptr);
		FLOAT_TYPE* dpsi_rel_dev_ptr = (dpsi_rel_ptr);
		FLOAT_TYPE* dUx_dev_ptr = (dUx_ptr);
		FLOAT_TYPE* dUy_dev_ptr = (dUy_ptr);
		FLOAT_TYPE* dV_dev_ptr = (dV_ptr);
		FLOAT_TYPE* dr_dev_ptr = (dr_ptr);
		...
		const FLOAT_TYPE* x_rel_dev_ptr = (x_rel_ptr);
		const FLOAT_TYPE* y_rel_dev_ptr = (y_rel_ptr);
		const FLOAT_TYPE* psi_rel_dev_ptr = (psi_rel_ptr);
		const FLOAT_TYPE* Ux_dev_ptr = (Ux_ptr);
		const FLOAT_TYPE* Uy_dev_ptr = (Uy_ptr);
		const FLOAT_TYPE* V_dev_ptr = (V_ptr);
		const FLOAT_TYPE* r_dev_ptr = (r_ptr);
		...
		for (size_t index = 0; index < x_rel_uvec.size(); ++index) {
			const FLOAT_TYPE x_rel = x_rel_dev_ptr[index];
			const FLOAT_TYPE y_rel = y_rel_dev_ptr[index];
			const FLOAT_TYPE psi_rel = psi_rel_dev_ptr[index];
			const FLOAT_TYPE Ux = Ux_dev_ptr[index];
			const FLOAT_TYPE Uy = Uy_dev_ptr[index];
			const FLOAT_TYPE V = V_dev_ptr[index];
			const FLOAT_TYPE r = r_dev_ptr[index];
			...
			FLOAT_TYPE cos_psi_rel;
			sincos_float_type<FLOAT_TYPE>(psi_rel, sin_psi_rel, cos_psi_rel);
			dx_rel_dev_ptr[index] = V * cos_psi_rel - Ux + y_rel * r;    // dx_rel
			dy_rel_dev_ptr[index] = V * sin_psi_rel - Uy - x_rel * r;    // dy_rel
		}
		...
		if (beacls::is_cuda(d_uvec) && beacls::is_cuda(Fx_uvec)) {
			const FLOAT_TYPE* d_dev_ptr = (d_ptr);
			const FLOAT_TYPE* Fx_dev_ptr = (Fx_ptr);
			beacls::synchronizeUVec(d_uvec);
			// beacls::synchronizeUVec(Fx_uvec);
			for (size_t index = 0; index < dUx_uvec.size(); ++index) {
				const FLOAT_TYPE Ux = Ux_dev_ptr[index];
				const FLOAT_TYPE Uy = Uy_dev_ptr[index];
				const FLOAT_TYPE r = r_dev_ptr[index];
				const FLOAT_TYPE d = d_dev_ptr[index];
				const FLOAT_TYPE Fx = Fx_dev_ptr[index];
				FLOAT_TYPE Fxf = (Fx >= 0)*Fx*X1::fwd_frac + (Fx < 0)*Fx*X1::fwb_frac;    // no branches! (... apparently this is unnecessary)
				FLOAT_TYPE Fxr = (Fx >= 0)*Fx*X1::rwd_frac + (Fx < 0)*Fx*X1::rwb_frac;    // no branches!
				FLOAT_TYPE Fx_drag = -X1::Cd0 - Ux * (X1::Cd1 + X1::Cd2 * Ux);
				FLOAT_TYPE af = atan2_float_type<FLOAT_TYPE>(Uy + X1::a * r, Ux) - d;
				...
				ratio = abs_float_type<FLOAT_TYPE>(tana * Ca / (3 * (Fymax > 0) * Fymax + (Fymax <= 0))) + (Fymax <= 0); // ratio >= 1 if Fymax <= 0
				FLOAT_TYPE Fyr = (ratio < 1)*(-Ca * tana * (1 - ratio + ratio * ratio / 3)) + (ratio >= 1)*(-copysign_float_type<FLOAT_TYPE>(Fymax, tana));
				dUx_dev_ptr[index] = (Fx + Fx_drag) / X1::m + r * Uy;          // dUx
				dUy_dev_ptr[index] = (Fyf + Fyr) / X1::m - r * Ux;             // dUy
				dr_dev_ptr[index] = (X1::a * Fyf - X1::b * Fyr) / X1::Izz;    // dr
			}
		}
		else {
			std::cerr << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << " Invalid data size" << std::endl;
		...
		if (beacls::is_cuda(w_uvec) && beacls::is_cuda(a_uvec)) {
			const FLOAT_TYPE* w_dev_ptr = (w_ptr);
			const FLOAT_TYPE* a_dev_ptr = (a_ptr);
			beacls::synchronizeUVec(w_uvec);    // not in PlaneCAvoid_cuda.cu
												// beacls::synchronizeUVec(a_uvec);    // not in PlaneCAvoid_cuda.cu
			for (size_t index = 0; index < psi_rel_uvec.size(); ++index) {
				const FLOAT_TYPE r = r_dev_ptr[index];
				const FLOAT_TYPE w = w_dev_ptr[index];
				const FLOAT_TYPE a = a_dev_ptr[index];
				dpsi_rel_dev_ptr[index] = w - r;    // dpsi_rel
				dV_dev_ptr[index] = a;        // dV
			}
		}
	...
	bool dynamics_cell_helper_execute_cuda(
		...
		FLOAT_TYPE* dx_dim_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(dx_uvec).ptr());
		// not sure if doing all casts up front like this is over-transferring memory
		const FLOAT_TYPE* x_rel_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[0]).ptr());
		const FLOAT_TYPE* y_rel_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[1]).ptr());
		const FLOAT_TYPE* psi_rel_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[2]).ptr());
		const FLOAT_TYPE* Ux_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[3]).ptr());
		const FLOAT_TYPE* Uy_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[4]).ptr());
		const FLOAT_TYPE* V_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[5]).ptr());
		const FLOAT_TYPE* r_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(x_uvecs[6]).ptr());
		const beacls::UVec& d_uvec = u_uvecs[0];
		const beacls::UVec& Fx_uvec = u_uvecs[1];
		const beacls::UVec& w_uvec = d_uvecs[0];
		const beacls::UVec& a_uvec = d_uvecs[1];
		...
		case 0:    // dx_rel
		{
			for (size_t index = 0; index < dx_uvec.size(); ++index) {
				const FLOAT_TYPE y_rel = y_rel_dev_ptr[index];
				const FLOAT_TYPE psi_rel = psi_rel_dev_ptr[index];
				const FLOAT_TYPE Ux = Ux_dev_ptr[index];
				const FLOAT_TYPE V = V_dev_ptr[index];
				...
				dx_dim_dev_ptr[index] = V * cos_float_type<FLOAT_TYPE>(psi_rel) - Ux + y_rel * r;    // dx_rel
			}
		}
		break;
		case 1:    // dy_rel
		{
			for (size_t index = 0; index < dx_uvec.size(); ++index) {
				...
				const FLOAT_TYPE V = V_dev_ptr[index];
				const FLOAT_TYPE r = r_dev_ptr[index];
				dx_dim_dev_ptr[index] = V * sin_float_type<FLOAT_TYPE>(psi_rel) - Uy - x_rel * r;    // dy_rel
			}
		}
		break;
		case 2:    // dpsi_rel
			if (beacls::is_cuda(w_uvec)) {
				const FLOAT_TYPE* w_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(w_uvec).ptr());
				beacls::synchronizeUVec(w_uvec);
				for (size_t index = 0; index < dx_uvec.size(); ++index) {
					const FLOAT_TYPE r = r_dev_ptr[index];
					const FLOAT_TYPE w = w_dev_ptr[index];
					dx_dim_dev_ptr[index] = w - r;    // dpsi_rel
				}
			}
		...
		case 3:    // dUx
			if (beacls::is_cuda(Fx_uvec)) {
				const FLOAT_TYPE* Fx_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(Fx_uvec).ptr());
				beacls::synchronizeUVec(Fx_uvec);
				for (size_t index = 0; index < dx_uvec.size(); ++index) {
					const FLOAT_TYPE Ux = Ux_dev_ptr[index];
					const FLOAT_TYPE Uy = Uy_dev_ptr[index];
					...
					const FLOAT_TYPE Fx = Fx_dev_ptr[index];
					FLOAT_TYPE Fx_drag = -X1::Cd0 - Ux * (X1::Cd1 + X1::Cd2 * Ux);
					dx_dim_dev_ptr[index] = (Fx + Fx_drag) / X1::m + r * Uy;    // dUx
				}
			}
		...
		case 4:    // dUy
			if (beacls::is_cuda(d_uvec) && beacls::is_cuda(Fx_uvec)) {
				const FLOAT_TYPE* d_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(d_uvec).ptr());
				const FLOAT_TYPE* Fx_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(Fx_uvec).ptr());
				beacls::synchronizeUVec(d_uvec);
				beacls::synchronizeUVec(Fx_uvec);
				for (size_t index = 0; index < dx_uvec.size(); ++index) {
					...
					ratio = abs_float_type<FLOAT_TYPE>(tana * Ca / (3 * (Fymax > 0) * Fymax + (Fymax <= 0))) + (Fymax <= 0); // ratio >= 1 if Fymax <= 0
					FLOAT_TYPE Fyr = (ratio < 1)*(-Ca * tana * (1 - ratio + ratio * ratio / 3)) + (ratio >= 1)*(-copysign_float_type<FLOAT_TYPE>(Fymax, tana));
					dx_dim_dev_ptr[index] = (Fyf + Fyr) / X1::m - r * Ux;    // dUy
				}
			}
		...
		case 5:    // dV
			if (beacls::is_cuda(a_uvec)) {
				const FLOAT_TYPE* a_dev_ptr = (beacls::UVec_<FLOAT_TYPE>(a_uvec).ptr());
				beacls::synchronizeUVec(a_uvec);
				for (size_t index = 0; index < dx_uvec.size(); ++index) {
					const FLOAT_TYPE a = a_dev_ptr[index];
					dx_dim_dev_ptr[index] = a;    // dV
				}
			}
		...
		case 6:    // dr
			if (beacls::is_cuda(d_uvec) && beacls::is_cuda(Fx_uvec)) {
				...
				for (size_t index = 0; index < dx_uvec.size(); ++index) {
					...
					ratio = abs_float_type<FLOAT_TYPE>(tana * Ca / (3 * (Fymax > 0) * Fymax + (Fymax <= 0))) + (Fymax <= 0); // ratio >= 1 if Fymax <= 0
					FLOAT_TYPE Fyr = (ratio < 1)*(-Ca * tana * (1 - ratio + ratio * ratio / 3)) + (ratio >= 1)*(-copysign_float_type<FLOAT_TYPE>(Fymax, tana));
					dx_dim_dev_ptr[index] = (X1::a * Fyf - X1::b * Fyr) / X1::Izz;    // dr
				}
			}
			else if (!beacls::is_cuda(d_uvec) && !beacls::is_cuda(Fx_uvec)) {
				const FLOAT_TYPE d = beacls::UVec_<FLOAT_TYPE>(d_uvec).ptr()[0];
